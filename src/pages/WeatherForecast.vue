<script setup lang="ts">
  ...
  const isCurrentHour = (forecastTime: string) => {
    ...
    <div class="flex flex-col max-w-3xl justify-center min-h-screen">
      ...
      <div v-if="dailyForecast.length" class="mt-4">
        ...
        <div class="grid grid-cols-6 gap-y-2 gap-x-4">
          <div v-for="forecast in dailyForecast" :key="forecast.time"
            :class="['mt-2', 'bg-gray-600', 'p-4', 'rounded-lg', { 'bg-gray-400': isCurrentHour(forecast.time) }]">
